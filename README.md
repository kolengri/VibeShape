...
The repository contains the **research and specification** plus an executable Phase 1 foundation scaffold. The Bun monorepo, shared TypeScript environments, quality gates, CI, Vite application shell, typed ICU localization, Tailwind tokens, and first source-owned shadcn primitives are operational. The domain package now provides a small production-oriented foundation: strict UUIDv7 identifiers, canonical millimeter/radian/scalar quantities with retained source-unit metadata, revisioned document and feature commands, deterministic events and replay, revision-safe actor-bound drafts, validated command and feature-type module descriptors, and a strict feature-DAG/evaluation seam. Feature add, full-record update, and suppression commands validate the resulting graph atomically and preserve prior values in replayable events. The first-party part-design module registers unit-aware box and cylinder plus ordered two-input Boolean/Subtract schemas through exact module, version, type, and schema identities; registry-bound trusted add/update handlers perform a pure document preflight, reject unavailable or invalid types, normalize parameters, and only then produce the final event. Each handler also produces bounded semantic content parameters, so equivalent source units share one canonical evaluation identity. That identity orders dependency hashes by declared input slots, replaces dependency UUIDs in topology references with slot indices, includes the exact host, geometry, kernel, tolerance, and provider identities, and excludes feature UUID, label, suppression, and retained input-unit presentation metadata. SHA-256 remains an injected domain port. Protocol v7 gives the worker an independent runtime boundary that reserializes and verifies the canonical environment, digest, and ordered dependency slots before OCCT execution. It evaluates box, cylinder, and Boolean subtraction, validates one positive-volume solid, emits topology candidates and transferable meshes, and reuses exact document-owned B-Rep entries by content hash. Boolean evaluation resolves only exact same-document input shapes; invalid replacement geometry is released while the prior valid result remains reusable. Handler composition rejects a registry built from different feature descriptors. Structural replay and suppression remain available for preserved unavailable features. The graph rejects invalid, duplicate, missing, cyclic, and undeclared topology dependencies; rebuild evaluation propagates dirtiness, reuses independent results, blocks only dependent failure branches, and contains evaluator failures as stable diagnostics. Trusted dispatchers fail closed when executable handlers drift from their descriptors. `@vibeshape/automation-api` provides strict lifecycle schemas and a bounded, revision-tagged document-summary view; `@vibeshape/automation-host` coordinates host-generated, owner-bound, expiring disposable drafts over injected document ports and the ordinary query and command dispatchers, including the first non-geometric feature-command fixture. There is no MCP transport or SDK dependency. This is not yet a complete CAD document model, document-connected feature evaluator, production persistence integration, undo stack, paired automation session, or public extension API. SPK-001 through SPK-005 clear the controlled OCCT worker, SolveSpace solver, stable topology, minimal 3MF interoperability, and semantic persistence/recovery gates. SPK-006 proceeds with reduced scope: immutable exact-integrity packages, no-import WebAssembly feature fixtures, capabilities, restricted states, and opaque iframe UI pass in Chromium, Firefox, and WebKit, while arbitrary same-origin workspace JavaScript is rejected. Generated native artifacts and spike adapters remain quarantined until their production integrations and release gates are reviewed. Sketch domain integration, `.vshape`, and general production geometry, file, persistence, and extension workflows remain unimplemented.
...
2. Connect the revisioned feature DAG to the protocol-v7 box/cylinder/Boolean worker path, then add topology repair events and save/reopen rebuild coverage over the accepted `TopoRef` and downstream-failure contracts.
