...
The pure domain layer provides UUIDv7 identities, canonical quantities, revisioned project display-unit preferences, revisioned document, sketch, variable, and feature commands, deterministic events and replay, actor-bound drafts, validated module and feature-type registries, atomic whole-DAG mutations, canonical feature-content identity, and sequential asynchronous rebuild scheduling. Sketch schema v0 stores bounded analytical point, line, circle, circular-arc, full-ellipse, and elliptical-arc entities on an origin plane plus every P0 constraint family; dimensional constraints retain authored Quantity expressions such as `#width`. The first-party part-design module contributes unit-aware box, cylinder, ordered Boolean/Subtract, and selector-backed extrusion with explicit new/add/remove/intersect body semantics. `@vibeshape/application` accepts a committed `DocumentSnapshot`, validates its DAG, derives changed roots from the previous revision, and composes the scheduler, injected SHA-256 hashing, validated derived state, transient feature-content preparation, and a serializable geometry port into a fail-closed rebuild use case.

Document protocol v16 sends a runtime-validated committed snapshot, including project display units, authored variables, sketches, stable planar-face sketch supports, and stable model vertex, linear-edge, circular-edge, elliptical-edge, and planar-intersection references, to `@vibeshape/document-worker`; permits exact-revision solving of a complete non-persisted sketch draft; carries compound signed line-chain Offset constraints and typed orphan reference intent; and transfers bounded saved-sketch display geometry and selector-backed closed profiles projected onto exact support frames. The worker owns dimensional expression evaluation, trusted feature-parameter resolution, incremental rebuild state, generation checks, sequential scheduling, SHA-256 hashing, the application coordinator, the OCCT engine, and the production sketch-solve boundary for the document. Geometry protocol v12 additionally carries bounded transient analytical loops, ordered topology references, exact right-handed world frames for extrusion, and disposable exact vertex, linear-edge, circle-edge, arc-edge, ellipse-edge, and elliptical-arc-edge reference geometry without persisting solved coordinates, candidate IDs, native topology IDs, or tessellation face IDs. The worker exports exact terminal B-Rep bodies as transferable STEP or binary STL, or retessellates each exact body with fixed print tolerances for deterministic multi-object 3MF, and prunes removed, suppressed, failed, or superseded native shapes after every successful rebuild.
