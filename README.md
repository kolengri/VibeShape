...
Document protocol v1 now sends a runtime-validated committed snapshot to `@vibeshape/document-worker`. That worker owns incremental rebuild state, generation checks, sequential scheduling, SHA-256 hashing, the application coordinator, and the OCCT engine for the document. A document-scoped browser session detects worker errors, message deserialization failures, and request timeouts; it replaces the worker, increments the generation, and retries one recoverable rebuild from the latest successfully rebuilt semantic snapshot. Protocol v7 still independently reserializes and verifies each canonical feature environment, digest, and ordered dependency slot before OCCT execution. The browser integration harness proves the full main-thread session → document worker → domain DAG → application coordinator → OCCT path, including clean reuse, hard worker replacement, full recovery rebuild, changed-descendant rebuild after recovery, transferable mesh clones, health reporting, and document-scoped disposal. Page-reload and save/reopen geometry recovery from persistence, persistent caches, topology repair events, user-driven hard cancellation, undo/redo, and product persistence orchestration remain open.
