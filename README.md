...
Document protocol v1 now sends a runtime-validated committed snapshot to `@vibeshape/document-worker`. That worker owns incremental rebuild state, generation checks, sequential scheduling, SHA-256 hashing, the application coordinator, and the OCCT engine for the document. Protocol v7 still independently reserializes and verifies each canonical feature environment, digest, and ordered dependency slot before OCCT execution. The browser integration harness proves the full main-thread client → document worker → domain DAG → application coordinator → OCCT path, including clean reuse, changed-descendant rebuild, transferable mesh clones, health reporting, and document-scoped disposal. Save/reopen geometry recovery, persistent caches, topology repair events, hard cancellation, undo/redo, and product persistence orchestration remain open.
...
2. Integrate the document worker with committed persistence and restart recovery, then add topology repair events and save/reopen coverage over the accepted `TopoRef` and downstream-failure contracts.
