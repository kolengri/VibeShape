...
The pure domain layer provides UUIDv7 identities, canonical quantities, revisioned document and feature commands, deterministic events and replay, actor-bound drafts, validated module and feature-type registries, atomic whole-DAG mutations, canonical feature-content identity, and sequential asynchronous rebuild scheduling. The first-party part-design module contributes unit-aware box, cylinder, and ordered Boolean/Subtract features. `@vibeshape/application` now composes the DAG scheduler, injected SHA-256 hashing, validated previous derived geometry, and a serializable geometry port into a fail-closed rebuild use case. It reuses clean independent results, rebuilds changed descendants, contains rejected ports as stable diagnostics, and exposes only geometry whose content hash matches a final successful record.

Protocol v7 independently reserializes and verifies the canonical environment, digest, and ordered dependency slots before OCCT execution. The worker evaluates box, cylinder, and Boolean subtraction, validates one positive-volume solid, emits topology candidates and transferable meshes, and owns exact per-document B-Rep entries transactionally. A browser integration harness proves the full domain DAG → application coordinator → protocol client → OCCT worker path, including clean reuse and changed-descendant rebuild. Production document-worker hosting, save/reopen geometry recovery, persistent caches, topology repair events, undo/redo, and product persistence orchestration remain open.
...
2. Host the implemented graph rebuild coordinator in the production document worker, then add topology repair events and save/reopen coverage over the accepted `TopoRef` and downstream-failure contracts.
