...
The local-project library now reads bounded, strict IndexedDB summaries, identifies the current project, creates a new project, switches between existing projects by stable document ID, and permanently deletes an explicitly confirmed inactive project in one revision- and lease-checked transaction. Active-project deletion is blocked until the user switches away. Preview cards and duplication remain required before the P0 library is complete.
