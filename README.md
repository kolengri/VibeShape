...
Document protocol v11 sends a runtime-validated committed snapshot, including project display units, authored variables, sketches, stable planar-face sketch supports, and stable model vertex/LINE-edge references, to `@vibeshape/document-worker`; permits exact-revision solving of a complete non-persisted sketch draft; carries compound signed line-chain Offset constraints; and transfers bounded saved-sketch display geometry projected onto exact support frames. The worker owns dimensional expression evaluation, trusted feature-parameter resolution, incremental rebuild state, generation checks, sequential scheduling, SHA-256 hashing, the application coordinator, the OCCT engine, and the production sketch-solve boundary for the document. Geometry protocol v10 additionally carries bounded transient analytical loops, ordered topology references, exact right-handed world frames for extrusion, and disposable exact vertex/LINE-edge reference geometry without persisting solved coordinates, candidate IDs, native topology IDs, or tessellation face IDs. The worker exports exact terminal B-Rep bodies as transferable STEP or binary STL, or retessellates each exact body with fixed print tolerances for deterministic multi-object 3MF, and prunes removed, suppressed, failed, or superseded native shapes after every successful rebuild.
