...
import type { SolveSketchRecordResult } from "@vibeshape/sketch-solver"
import { type SketchSolveCache, solveSketchOnce } from "./extrusion-content"
import type { SketchSolvePort } from "./runtime"
import { type SupportFrame, sketchFrame } from "./support-frame"

type SketchDisplayRecord = Extract<
...
function solvedGeometry(sketch: SketchRecord, result: SolveSketchRecordResult | null) {
  const solution = result?.ok ? result.solution : null
...
function sampledCircle(center: SketchPoint2, radius: number): readonly SketchPoint2[] {
...
function displayRecord(
  ...
  sketch: SketchRecord,
  result: SolveSketchRecordResult | null,
): SketchDisplayRecord | null {
  const frame = sketchFrame(sketch, document, document.features)
  if (!frame) return null
  const { points, radii } = solvedGeometry(sketch, result)
...
export async function createSketchDisplayRecords(
  ...
  for (const sketch of document.sketches) {
    let result: SolveSketchRecordResult | null = null
    if (solveSketch) {
      try {
    ...
    const record = displayRecord(document, sketch, result)
