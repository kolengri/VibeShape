...
| [OpenCascade.js binding generator](https://github.com/donalffons/opencascade.js/blob/5ff2b750ba4b9a9fdfbff8842712cbb562e78ce7/src/bindings.py) | Emits a no-op Embind `raw_destructor<T>` for classes with a non-public destructor or two-argument placement delete; SPK-001 isolates this policy as the retained-allocation source for bound OCCT primitives |
...
**Conclusion:** OCCT is the only verified primary candidate for exact B-Rep modeling and STEP exchange. SPK-001 confirms that Replicad accelerates the required browser operations and that the controlled allocator-instrumented candidate builds from recorded sources. Native scoped lifetimes are allocation-neutral, but the pinned OpenCascade.js generator disables native destruction for OCCT placement-delete classes. The binding remains behind an adapter until a source-rebuilt generator correction establishes a plateau and topology-history coverage passes. See [SPK-001 evidence](spikes/spk-001-occt-worker.md).
