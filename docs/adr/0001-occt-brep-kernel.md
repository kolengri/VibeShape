...
[SPK-001](../spikes/spk-001-occt-worker.md) proves the required Replicad modeling, transient boolean and fillet history, tessellation, STEP round-trip, STL operations, stage memory checkpoints, hard worker restart, local controlled performance budget, independent-application STEP import, and technical corresponding-source bundle. Its functional matrix passes in Chromium, Firefox, and WebKit. Its controlled candidate is source-built from verified archives with the reviewed OpenCascade.js destructor correction and exposes allocator metrics. Purpose-owned OCCT adapters record zero retained bytes in every 1,000-operation lifecycle block and 448 bytes of post-warmup drift across four further complete batches. Twenty cold workers on the declared Apple M1 baseline reach a 178.5 ms initialization p95 and 278.8 ms complete-fixture p95 without an observed main-thread long task. Headless FreeCAD 1.1.3 imports the exact browser output as one valid solid with matching volume and bounds. This clears the Phase 0 stop/go gate; stable topology remains governed by ADR-0003 and SPK-003. This ADR remains accepted only for the spike until the production facade is selected.
