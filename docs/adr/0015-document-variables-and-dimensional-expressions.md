...
- The first feature integration retains Box expressions such as `20 mm` or `#width`, validates their resolved dimension and range adjacent to the owning field, and persists the authored source through ordinary feature-add and full-record feature-update commands. Existing-feature edit restores source expressions instead of serializing only resolved millimeter values.
