...
The rework harness under `native/occt` pins and verifies source archives for OpenCascade.js `5ff2b750`, OCCT `bb368e27`, and the Replicad `0.23.0` build config, along with an immutable builder image digest. The controlled artifact has been built and regression-tested, but this does not retroactively establish the source of the published npm WASM. Operation-isolated evidence requires a reviewed OpenCascade.js destructor-generator patch. The builder image must be reproduced from archived sources, with that patch, license texts, replacement instructions, and the exact manifest archived before release provenance is considered closed.
