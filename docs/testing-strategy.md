...
The domain feature-graph suite verifies presentation versus evaluation order, duplicate and missing identities, cycles, declared topology dependencies, first-run asynchronous evaluation, transitive dirty propagation, independent cache reuse, suppression, dependent-only failure blocking, independent branch progress, stable rejected-evaluator containment, and fail-closed dirty/cache inputs. Application tests cover ordered canonical geometry requests, complete snapshot validation, environment and mesh-policy invalidation, clean reuse, descendant-only rebuild, independent branch continuation, adapter-environment mismatch, and stale-geometry exclusion. Protocol conformance proves that domain identities and the dependency-independent worker serialization are byte-identical; protocol-v7 tests also reject reordered hashes, missing slots, and duplicate dependency feature IDs. Worker unit tests cover runtime validation, environment and digest rejection before engine execution, typed geometry failures, logical cancellation, transferable buffers, exact feature-cache reuse, transactional replacement, failed cleanup visibility, ordered dependency resolution, and document isolation. The primitive feature Playwright scenario executes the actual worker and OCCT in every target browser and verifies shape, topology, mesh, cache, rollback, ownership, and disposal invariants. The feature rebuild scenario additionally executes the domain DAG through the application coordinator and web adapter, proves zero worker calls for a clean rebuild, and evaluates only a changed cylinder plus its Boolean descendant. Production document-worker hosting, persisted Boolean lineage, and document save/reopen remain later integration suites.
...
The current executable evidence covers the foundation shell; the application graph rebuild coordinator and its real-browser OCCT path; the protocol-v7 box, cylinder, and dependency-aware Boolean/Subtract boundary; SPK-001 worker ownership, operations, memory, performance, exchange, and restart; SPK-003 semantic and composed topology resolution; SPK-004 deterministic 3MF and slicer invariants; SPK-005 transactional recovery and offline fallback; and the reduced-scope SPK-006 package and isolation boundary. It does not yet prove production document-worker hosting, document-integrated topology repair, a complete production CAD workflow, a real two-build service-worker update, `.vshape` round-trip, installed-build release behavior, or production extension execution.
...
The current conformance fixtures cover strict input rejection, exact-revision reads, bounded semantic summary output, command and feature-type descriptor-handler parity, command rejection on active feature-type composition drift, exact feature-type identity and unavailable-type preservation, canonical metric/imperial/angle/scalar normalization, bounded box/cylinder and Boolean/Subtract parameter validation, parameter and semantic-content normalizer failure containment, registry-bound add/update rejection before event creation, canonical JSON ordering, unit-equivalent and UUID-independent feature-content identity, dependency-slot and topology-reference mapping, exact runtime/provider identity, injected and worker-side digest validation, exact dependency-aware worker cache comparison, unavailable-feature suppression, owner and document isolation, host-generated draft identity, multi-command preview and commit, whole-DAG-validated feature add/update/suppression with deterministic replay and tamper rejection, inactivity renewal and expiry, idempotent discard, count limits, duplicate command rejection, concurrent operation serialization, stale atomic commit retention, generic feature-event persistence acceptance, and contained port failure. Application rebuild tests additionally cover asynchronous dependency sequencing, ordered canonical geometry requests, clean-result and geometry reuse, descendant-only invalidation, failure containment, independent-branch continuation, stale-geometry exclusion, and fail-closed previous-geometry validation. A real-browser harness runs the domain DAG through the application coordinator, web adapter, protocol v7 client, and OCCT worker, then proves clean reuse and a changed Boolean descendant. Production document-worker hosting, save/reopen rebuilds, pagination, derived views, expression evaluation, broader geometry eligibility, idempotent replay results, product persistence orchestration, feature deletion, undo/redo, confirmation, pairing, and browser disconnect behavior stay open until their executable contracts exist.
