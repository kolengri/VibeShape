...
The current conformance fixtures cover strict input rejection, exact-revision reads, bounded semantic summary output, command and feature-type descriptor-handler parity, exact feature-type identity and unavailable-type preservation, canonical metric/imperial/angle/scalar normalization, bounded box/cylinder parameter validation, normalizer failure containment, owner and document isolation, host-generated draft identity, multi-command preview and commit, whole-DAG-validated feature add/update/suppression with deterministic replay and tamper rejection, inactivity renewal and expiry, idempotent discard, count limits, duplicate command rejection, concurrent operation serialization, stale atomic commit retention, generic feature-event persistence acceptance, and contained port failure. Pagination, derived views, expression evaluation, command eligibility wiring, geometry evaluation, progress, cancellation, idempotent replay results, product persistence orchestration, feature deletion, undo/redo, confirmation, pairing, and browser disconnect behavior stay open until their executable contracts exist.
