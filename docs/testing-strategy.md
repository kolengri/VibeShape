...
- React component tests verify lazy adapter ownership, terminal-mesh filtering, exact historical and unsaved-preview feature highlighting from tree preselection and active edit, face, origin-plane, and graphical sketch-reference callback wiring, empty-model datum initialization, initial and explicit fit behavior, camera preservation across mesh replacement, sketch continuation and drag-target forwarding, cross-support point and line projection, read-only external-line selection, last-valid solved-display retention, disposal, and localized renderer failure containment.
