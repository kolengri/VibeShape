...
External-inference tests additionally require exact point projection onto analytical circles and bounded
positive-sweep arcs, deterministic line-over-curve priority, Shift suppression, source highlighting, and
commit-only materialization. Component coverage proves that Point creates one stable external curve and
one Point on curve constraint, while Line keeps its first placement provisional and materializes the same
