...
The external-sketch context suite requires a new unsaved draft to include every committed sketch while an existing draft excludes itself and later sketches. Browser coverage also proves that a source can be selected graphically from the unsaved target and committed as a stable external reference.
