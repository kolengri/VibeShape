...
Create a variable-driven rectangle and exact new-body extrusion, edit its driving variable after reopen, combine primitive solids with a Boolean, recover the project offline, round-trip a `.vshape` backup into fresh browser storage, and export 3MF/STEP/STL.

## Phase 2 — sketcher vertical slice (6–10 weeks)

The production boundary and first interaction slice are complete for rectangle profiles and exact new-body extrusion: origin-plane sketch records, all P0 constraint schemas, revisioned add/update/remove events, `.vshape` preservation, protocol v5 solving and profile results, geometry protocol v8 analytical profile transport, exact reviewed runtime verification, conflict/status mapping, variable dimensions, stable-ID continuation, deterministic outer/hole/island extraction, fail-closed stable selectors, OCCT prism construction, stable cap/side roles, worker recovery, the initial 1,000-point browser budget, and accessible rectangle and extrusion create/edit. The remaining phase is free-form entity interaction, general profile picking, add/remove/intersect body semantics, interactive solid preview, conflict editing, and general intersection splitting below.
...
- Unit-aware inputs.
- Extend the implemented expression-capable rectangle create/edit pattern to free-form sketch dimensions and additional feature types, including variable-reference insertion rather than manual typing alone.
