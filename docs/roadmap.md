...
SPK-001 now has executable functional evidence and remains **Rework** with one missing interoperability gate. The project source-builds the pinned builder with the reviewed destructor correction, verifies transient boolean and fillet history relations, reaches a 448-byte post-warmup allocator drift with zero retained bytes in every 1,000-operation lifecycle block, passes its local controlled Chromium performance budgets on the declared Apple M1 baseline, and creates a verified corresponding-source bundle. Independent STEP validation remains blocking. Stable `TopoRef` resolution remains SPK-003 work. See [SPK-001 evidence](spikes/spk-001-occt-worker.md).
