...
The first production-oriented domain slice implements feature schema v0, deterministic graph validation and ordering, transitive rebuild planning, independent cache reuse, conservative suppression, dependent-only failure propagation, and sequential asynchronous evaluator calls. Feature add, full-record update, and suppression changes participate in the ordinary document revision, draft, event replay, module-dispatch, automation-host, and persistence-schema paths. Canonical quantity v0 normalizes `um`, `mm`, `cm`, `m`, `in`, and `ft` to millimeters, degrees to radians, and dimensionless values to scalar identity while retaining strict source metadata. A first-party feature-type registry binds exact module/type/schema identities to trusted parameter and semantic-content normalizers; box, cylinder, and ordered two-input Boolean/Subtract are its initial descriptors. Registry-bound add/update handlers preflight document invariants and whole-DAG validity, validate and normalize the contributed type, prove that its bounded content projection is available, and only then emit the final event; suppression deliberately remains available for structurally preserved unavailable features. The domain assembles a versioned canonical evaluation identity from semantic parameters, ordered dependency hashes, slot-relative topology references, exact runtime/provider identities, and tolerance policy, then validates a digest from an injected SHA-256 port. Equivalent input units share identity while feature UUID, label, suppression, and source-unit presentation metadata do not affect geometry cache identity. `@vibeshape/application` now accepts a committed document snapshot, constructs its graph, automatically derives dirty roots from prior source features, and drives the scheduler through an injected geometry port. Its state is bound to document, revision, worker generation, environment, and mesh policy; it contains port failures and returns only authoritative matching geometry. Protocol v7 independently validates the wire identity, active geometry environment, SHA-256 digest, and exact ordered dependency slots; evaluates box, cylinder, and Boolean subtraction in OCCT; verifies one valid positive-volume solid; returns topology candidates and a transferable display mesh; and retains exact content-hash B-Rep entries under document-scoped ownership. Missing or mismatched input shapes fail closed, while invalid replacement geometry is disposed and leaves the prior valid Boolean result intact. The browser integration harness proves committed-document-to-worker execution, clean reuse, and automatic changed-descendant rebuild. Production document-worker hosting, persistent caches, save/reopen rebuilds, hard cancellation, topology repair, deletion policy, and undo integration remain the next gates.
