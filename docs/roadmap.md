...
The production-oriented foundation now spans strict revisioned document, variable, and feature schemas; deterministic replay and DAG scheduling; canonical geometry identity; worker-owned OCCT rebuild and STEP/STL export; transactional IndexedDB persistence; configurable Box, Cylinder, and Boolean/Subtract authoring and editing; dependency-safe feature deletion; variable refactoring; and a Three.js viewport with rendered-face selection. The native-file increment adds deterministic `.vshape` v0 backup and verified atomic import, retaining stable variable IDs, formulas, and feature source expressions while excluding derived geometry. The first project-library increment adds strict local summaries, current-project state, new-project creation, and existing-project switching. The browser harness proves recovery, worker replacement, selective rebuild, modeling edits, deletion, geometry export, native-project round-trip, and local-project switching. Project previews, duplication and confirmed deletion, body/edge/vertex selection, stable selection references, interactive preview, persistent caches, autosave scheduling, backup reminders and restore/copy UX, BroadcastChannel ownership, user-driven hard cancellation, topology repair, extension-specific variable refactor contributions, richer expressions, undo integration, print-quality export profiles and reports, and production 3MF integration remain the next gates.
