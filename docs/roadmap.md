...
The first production-oriented domain slice now implements feature schema v0, deterministic graph validation and ordering, transitive rebuild planning, independent cache reuse, conservative suppression, and dependent-only failure propagation. Feature add, full-record update, and suppression changes participate in the ordinary document revision, draft, event replay, module-dispatch, automation-host, and persistence-schema paths. Canonical quantity v0 normalizes `um`, `mm`, `cm`, `m`, `in`, and `ft` to millimeters, degrees to radians, and dimensionless values to scalar identity while retaining strict source metadata. A first-party feature-type registry binds exact module/type/schema identities to trusted parameter and semantic-content normalizers; box and cylinder are its initial unit-aware descriptors. Registry-bound add/update handlers now preflight document invariants and whole-DAG validity, validate and normalize the contributed type, prove that its bounded content projection is available, and only then emit the final event; suppression deliberately remains available for structurally preserved unavailable features. The domain can assemble a versioned canonical evaluation identity from semantic parameters, ordered dependency hashes, slot-relative topology references, exact runtime/provider identities, and tolerance policy, then validate a digest from an injected SHA-256 port. Equivalent input units share identity while feature UUID, label, suppression, and source-unit presentation metadata do not affect geometry cache identity. Protocol v6 adds the first production-oriented worker slice: it independently validates the primitive wire identity, active geometry environment, and SHA-256 digest; evaluates dependency-free box and cylinder features in OCCT; verifies one valid positive-volume solid; returns semantic topology candidates and a transferable display mesh; and retains exact content-hash B-Rep entries under document-scoped ownership. Domain-to-worker orchestration, dependent operations, persistent caches, save/reopen rebuilds, hard cancellation, deletion policy, and undo integration remain the next gates.
