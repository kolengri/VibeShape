...
- IndexedDB v1 History persistence, lease-gated legacy promotion, and `.vshape` v2.
...
The production boundary and core P0 interaction slice are complete for analytical profiles and exact new/add/remove/intersect Extrude and Revolve: origin-plane, stable planar Box/Cylinder/Extrusion-face, and first-class signed offset Datum Plane sketch supports, all P0 constraint schemas, revisioned add/update/remove events, `.vshape` preservation, document protocol v17 solving, profile results, and stable model vertex, linear-edge, circle-edge, arc-edge, ellipse-edge, and elliptical-arc-edge records, geometry protocol v12 analytical profile, exact-frame, and rebuild-local exact reference-geometry transport, exact reviewed runtime verification, conflict/status mapping, variable dimensions, stable-ID continuation, deterministic outer/hole/island extraction, fail-closed stable selectors, disposable OCCT prism/revolution tools and explicit-target Boolean construction, origin, stable graphical same-sketch line, and stable straight model-edge Revolve axes, stable cap/side/vertex/LINE-edge roles plus Cylinder rim roles, separate support, axis-source, and body dependencies, worker recovery, the initial 1,000-point browser budget, an accessible interactive SVG sketcher with center-origin families, exact full-ellipse and elliptical-arc authoring, full-ellipse axis-quadrant and generic perimeter inference plus bounded elliptical-arc perimeter inference across authored, earlier-sketch, and stable model geometry, profiles, direct profile promotion, exact line/arc/circle modification including signed line-chain Offset, adaptive streamed exact drag feedback, local draft undo/redo, selectable stable profile regions, graphical model-vertex and analytical line/circle/arc/ellipse/elliptical-arc Use in normal and orbit views, exact selection-first earlier-sketch line and stable linear model-edge Pierce across supports, feature/origin/datum visibility, and exact disposable feature previews. ADR-0034 delivers bounded same-sketch multi-profile New for Extrude and Revolve with canonical `Shift`-toggle selection, one sketch solve, deterministic analytical tool construction, bounded multi-solid results, and full edit/reopen retention. ADR-0036 and ADR-0037 extend Extrude and Revolve with target-first multi-profile Add, Remove, and Intersect, one combined tool, a one-solid result invariant, and downstream terminal-body reuse; Revolve additionally retains one exact shared axis and ordered axis-source dependency. The remaining phase includes stable per-solid New-result identity, spline and other non-analytical curved feature-geometry Use, curved Pierce, mid-plane, angular, three-point, axis, and point reference geometry, general planar-face repair, round-curve Offset, remaining round/ellipse modification boundaries, spline authoring, guided conflict repair, and general intersection splitting below.

The Extrusion-face support claim includes caps and line-generated planar sides with stable `extrusion.side.<sourceEntityId>` roles. Missing and ambiguous saved supports are identified in History and can enter graphical support replacement without changing sketch entity or constraint identities. General unannotated planar B-Rep faces remain open.
...
- Extend the implemented explicit-target Revolve beyond origin, same-sketch line, and stable straight model-edge axes with connector and curved axes plus additional end conditions; add a dedicated Pocket workflow after the shared preview and target-selection contracts stabilize.
