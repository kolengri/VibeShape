...
Protocol v6 implements the first worker-side consumer without importing the domain package. The worker revalidates and canonically serializes the dependency-free primitive identity, compares the exact active host/adapter/kernel/tolerance/provider environment, recomputes SHA-256 with Web Crypto, and only then invokes OCCT. Box and cylinder evaluation is transactional: a new shape must produce exactly one valid positive-volume solid and a successful tessellation before it replaces the prior feature entry. Successful entries are owned per document and feature and reused only on an exact content-hash match; document disposal releases only that document's feature shapes. The response carries B-Rep hit status, invariant measurements, semantic face candidates, timings, and four transferable mesh buffers. Domain scheduler orchestration, dependent operations, persistent cache records, hard interruption of synchronous OCCT, stale-geometry presentation, and repair events remain production integration work.
