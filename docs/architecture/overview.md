...
This structure is now checked in. `apps/web` renders a static, accessible CAD-shell placeholder through Vite, resolves typed product copy through `@vibeshape/i18n`, and proves Tailwind discovery across `@vibeshape/ui`. `packages/protocol` and `packages/geometry-worker` contain the protocol-v7 production-oriented box, cylinder, and dependency-aware Boolean evaluation seam alongside the isolated SPK-001 operations. `packages/domain` contains the first narrow document and feature-command slice, including canonical quantity schemas, strict document and feature records, whole-DAG validation, deterministic events and replay, revision-safe drafts, command and feature-type module descriptors, unit-aware box/cylinder and Boolean/Subtract schemas, asynchronous rebuild scheduling, and trusted registries. `packages/application` now accepts a committed document snapshot, validates and constructs its graph, derives changed roots from the prior feature records, and composes the pure scheduler, canonical content hashing, a serializable geometry port, previous-state validation, and authoritative derived-geometry selection. Rebuild state is bound to document, revision, worker generation, geometry environment, and mesh policy; a generation change rebuilds every native shape, while document or feature labels do not invalidate geometry. It is not a persistence transaction or a production document-worker host. `apps/web` supplies the initial geometry-client adapter and a browser integration harness; production composition must execute rebuild coordination outside the main thread under [ADR-0002](../adr/0002-geometry-worker-boundary.md). `packages/automation-api` contains strict draft-lifecycle schemas, the bounded revision-tagged document-summary view, and trusted query dispatch. `packages/automation-host` binds host-generated drafts to a complete actor identity, serializes lifecycle operations, enforces inactivity and count limits, previews through the query dispatcher, and commits document or feature records only through an injected atomic compare-and-commit port. `packages/formats` owns the SPK-004 deterministic 3MF Core writer and strict export report. `packages/persistence` owns the SPK-005 strict Dexie records, atomic repository, checksum recovery, writer leases, progressive storage capability probing, and verified disposable OPFS cache; its generic event and snapshot schemas accept feature revisions, but product persistence orchestration remains open. These packages do not yet provide general expression evaluation, undo/redo, production document-worker rebuild composition, save/reopen geometry recovery, application persistence orchestration, confirmation UI, paired automation sessions, `.vshape`, or production export orchestration. The viewer and print-analysis package entry points remain intentionally empty until their owning work introduces tested contracts and dependency evidence.
