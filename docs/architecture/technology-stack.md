...
SPK-001 proved the Replicad `0.23.1` path across the required modeling and exchange operations in all three automated browser engines. The controlled package is now built from verified OpenCascade.js and OCCT archives with a reviewed destructor correction and 262 configured bindings. VibeShape-owned OCCT adapters make primitive, boolean, fillet, tessellation, STEP, and STL temporary lifetimes deterministic. Controlled Chromium evidence records zero retained bytes in every 1,000-operation lifecycle block and 432 bytes of post-warmup drift across four further full batches. The candidate is not yet the production binding because operation-history coverage, independent STEP validation, declared performance budgets, and the release compliance bundle remain open. The measurements and rework decision are recorded in [SPK-001 evidence](../spikes/spk-001-occt-worker.md).
