...
SPK-001 proved the Replicad `0.23.1` path across the required modeling and exchange operations in all three automated browser engines. The controlled package is now built from verified OpenCascade.js and OCCT archives with a reviewed destructor correction and 262 configured bindings. VibeShape-owned OCCT adapters make primitive, boolean, fillet, history, tessellation, STEP, and STL temporary lifetimes deterministic. Controlled Chromium evidence records zero retained bytes in every 1,000-operation lifecycle block and 448 bytes of post-warmup drift across four further full batches. On the declared Apple M1 baseline, 20 cold workers reach a 178.5 ms initialization p95 and 278.8 ms complete-fixture p95 with no observed main-thread long tasks; peak WASM capacity is 20,185,088 bytes and peak live native allocation is 921,832 bytes. Boolean and fillet history APIs provide the transient relations needed to start SPK-003, but do not themselves solve stable topology. A verified local corresponding-source bundle closes the candidate's technical release-bundle gate. Headless FreeCAD 1.1.3 imports the exact browser-exported STEP file as one valid solid with matching volume and bounds, closing the independent-application interoperability gate while not claiming kernel diversity. The candidate passes the Phase 0 stop/go gate but is not yet the production binding because the facade comparison and extended corpus remain. The measurements and decision are recorded in [SPK-001 evidence](../spikes/spk-001-occt-worker.md).
...
The implemented `@vibeshape/viewer` adapter pins Three.js `0.185.1`, loads as a separate dynamic chunk only after authoritative geometry exists, then requires a WebGL2 context. It constructs `BufferGeometry` directly from worker-owned typed arrays and renders on OrbitControls changes or explicit data, selection, or size updates rather than running an idle animation loop. Its orthographic camera is Z-up and fits the combined terminal-feature bounds. A CPU `Raycaster` maps an indexed `faceIndex` through the corresponding `triangleFaceIds` entry, then extracts the matching rendered triangles for exact hover and selected-face overlays. Primary click is reserved for selection; middle and secondary drag own orbit and pan. Replacing a model clears ephemeral selection and disposes its surface, edge, and highlight geometries; unmount additionally disposes controls, shared materials, and the renderer. Triangle-to-face IDs never cross into persisted topology or domain-command identity.
