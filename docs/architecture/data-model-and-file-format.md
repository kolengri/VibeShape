...
- a bounded cross-support sketch-to-sketch external point-and-line reference foundation with stable source/projected IDs, exact source-frame-to-world-to-target-frame projection, read-only fixed solver geometry excluded from profiles, source-removal protection, and no serialized solved coordinates;
