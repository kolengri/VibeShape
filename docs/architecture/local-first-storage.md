...
SPK-005 implements the original six active stores: `projects`, `snapshots`, `events`, `recovery`, `leases`, and `cacheIndex`. The additive product schema v2 introduces `projectThumbnails` without rewriting existing stores or records. Imports, settings, and explicit migration history remain production additions. See [SPK-005 evidence](../spikes/spk-005-local-first.md).
