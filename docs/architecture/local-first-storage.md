...
The first product Projects dialog lists local projects, identifies the current one, creates a new project, and switches to an existing project after validating that its IndexedDB record still exists. It also implements the file fallback baseline in every browser: `Download .vshape` is distinct from STEP/STL geometry export, and `Choose .vshape` validates a bounded archive before importing it. Every successful project switch closes the prior session, selects the target document or clears the active selection for a new document, reloads, and rebuilds derived geometry. The current slice deliberately omits project previews, duplication, deletion, silent same-ID restore, copy-as-new identity rewriting, directory mirroring, and persistent file handles.
