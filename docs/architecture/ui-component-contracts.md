...
- a specialized SVG sketch canvas that receives one schema-valid analytical draft, delegates mutations, click-targeted line modification, and deterministic point, bounded-intersection, direction inference, and regular-polygon construction to pure domain operations, preserves retained line identity while repairing detached endpoint dependencies, persists accepted inference and polygon design intent as ordinary constraints, coalesces drag samples to one inference/render update per animation frame, exposes entity and stable-region selection, renders constraint and authored-dimension glyphs, and keeps solved display geometry disposable;
