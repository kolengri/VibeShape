...
- a specialized SVG sketch canvas that receives one schema-valid analytical draft, delegates mutations and deterministic point, bounded-intersection, and direction inference to pure domain operations, persists accepted inference as ordinary constraints, coalesces drag samples to one inference/render update per animation frame, exposes entity and stable-region selection, renders constraint and authored-dimension glyphs, and keeps solved display geometry disposable;
