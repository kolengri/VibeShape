...
- Rectangle placement adds horizontal and vertical intent automatically. Center Rectangle adds stable construction spokes plus the minimum equal/parallel intent needed to retain its picked center without over-constraining SolveSpace. Three-point Arc reuses inferred endpoint identities, previews the exact circumcircle after the second endpoint, and records its oriented analytical arc as one draft edit. Point and Line placement plus point dragging provide deterministic existing-point, line and circular-arc midpoint, bounded segment-intersection, point-on-line, distant horizontal/vertical point alignment, exact circle or bounded-arc quadrant inference, exact full-ellipse axis-quadrant and generic perimeter inference, and exact positive-sweep elliptical-arc perimeter inference; Line additionally previews and persists horizontal, vertical, parallel, perpendicular, or endpoint-tangent intent. Arc midpoint inference uses half the positive analytical sweep rather than the chord midpoint and persists dedicated midpoint intent that follows endpoint edits. Circle and arc quadrants retain point-on-curve intent plus horizontal/vertical alignment to the stable center. Ellipse quadrants persist one dedicated axis intent backed by an exact solver-owned trammel locus; arbitrary full-ellipse perimeter snaps persist the same exact locus without the axis selector, while elliptical arcs add a reviewed bounded-branch equation and post-solve sweep guard. `Shift` suppresses automatic inference for the current pointer sample. Applied geometric constraints and driving expressions render as viewport glyphs. Remembered wake-up references cover visible earlier-sketch geometry and uniquely resolved coplanar model points, lines, circular curves, full ellipses, elliptical arcs, and their stable centers without duplicating one materialized curve reference. Other projected geometry remains open. Live solver state and degrees of freedom are visible.
...
Status: stable saved-profile 3D picking and in-task graphical profile selection are implemented for Extrude and Revolve create/edit. Primary click replaces the selection and `Shift`-click toggles profiles from one sketch. New feature versions persist the canonical bounded set, solve the source sketch once, preview and build one analytical tool per profile, retain one-to-many valid solids, and reopen with the complete selection. Extrude and Revolve additionally support target-first multi-profile Add, Remove, and Intersect through one combined canonical tool and require exactly one valid result solid; each result can drive a later ordinary modifying feature. Revolve retains one exact shared origin, sketch-line, or model-edge axis. Cross-sketch sets and stable per-solid identity for multi-profile New remain open under [ADR-0034](../adr/0034-versioned-multi-profile-feature-input.md), [ADR-0036](../adr/0036-multi-profile-modifying-extrusion.md), and [ADR-0037](../adr/0037-multi-profile-modifying-revolve.md).
...
- Extend the implemented selector-backed Revolve from origin, same-sketch line, and stable straight model-edge axes to connector and curved axes, then add Hole, Fillet, Chamfer, Shell, Pattern, and Mirror as ordinary ordered features.
