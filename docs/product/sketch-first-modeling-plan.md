...
- Point, Line/Polyline, corner Rectangle, center Rectangle, Circle, center-point Arc, three-point Arc, and Construction modes author analytical geometry.
...
- Rectangle placement adds horizontal and vertical intent automatically. Center Rectangle adds stable construction spokes plus the minimum equal/parallel intent needed to retain its picked center without over-constraining SolveSpace. Three-point Arc reuses inferred endpoint identities, previews the exact circumcircle after the second endpoint, and records its oriented analytical arc as one draft edit. Point and Line placement plus point dragging provide deterministic existing-point, segment-midpoint, bounded segment-intersection, and point-on-line inference; Line additionally previews and persists horizontal, vertical, parallel, perpendicular, or endpoint-tangent intent. `Shift` suppresses automatic inference for the current pointer sample. Applied geometric constraints and driving expressions render as viewport glyphs; remembered wake-up references, point-to-point alignment, arc midpoint/quadrant inference, and projected geometry remain open. Live solver state and degrees of freedom are visible.
