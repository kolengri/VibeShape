...
Status: stable saved-profile 3D picking and in-task graphical profile selection are implemented for Extrude and Revolve create/edit. Primary click replaces the selection and `Shift`-click toggles profiles from one sketch. New feature versions persist the canonical bounded set, solve the source sketch once, preview and build one analytical tool per profile, retain one-to-many valid solids, and reopen with the complete selection. Extrude additionally supports target-first multi-profile Add, Remove, and Intersect through a combined canonical tool and requires exactly one valid result solid; that result can drive a later ordinary modifying feature. Multi-profile modifying Revolve, cross-sketch sets, and stable per-solid identity for multi-profile New remain open under [ADR-0034](../adr/0034-versioned-multi-profile-feature-input.md) and [ADR-0036](../adr/0036-multi-profile-modifying-extrusion.md).
