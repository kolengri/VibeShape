...
Before SKX-1, VibeShape implemented many of the necessary domain operations, but several primary loops left the viewport at their most important moment. Dimension collected entities on the drawing and then moved focus to `#sketch-dimension-expression` in the task panel. The first SKX-1 slice removed that DOM focus bridge: Dimension now collects geometry, previews witness leaders at the pointer, places an anchored variable-aware editor, preserves the clicked annotation location, supports staged `Escape`, edits an existing value by double-click, and moves a label as presentation-only state without solving or rebuilding. Selection-first constraint and dimension actions now appear beside the selected geometry rather than in a detached viewport corner. Right-clicking a selected entity uses the complete selection, while right-clicking an unselected entity targets only that explicit hit without changing selection merely by opening the menu. The labeled context menu invokes the same constraint, Dimension, and Delete behavior as the visible and keyboard-accessible paths. The task-panel field remains the accessible fallback. Reference dimensions and durable cross-session label-layout metadata remain follow-up work.
...
- Coplanar points, lines, analytical circles, and bounded arcs from earlier sketches can participate
  without first being selected from a list; supported model points, linear edges, and uniquely resolved
  circular or bounded-arc edges follow the same interaction.
...
- The Constraint manager is a filtered diagnostic surface for complex sketches. It is absent from the primary panel until the user opens it from the dedicated diagnostic action in the contextual Sketch toolbar, and it is not the primary creation workflow. Solver conflicts remain visible on the canvas and in the solve summary before the manager is opened.
