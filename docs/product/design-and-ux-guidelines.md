...
- Point and line placement use deterministic screen-tolerance inference. Existing-point snaps reuse the stable point identity; midpoint, bounded segment-intersection, point-on-line, distant point-to-point horizontal or vertical alignment, and exact circle or bounded-arc quadrant candidates persist the corresponding relations; and line direction candidates persist horizontal, vertical, parallel, perpendicular, or endpoint-tangent intent. A quadrant candidate preserves both Point on curve and horizontal or vertical alignment to the stable curve center. Point dragging accepts the same point-relation candidates in its single release commit. Holding `Shift` suppresses inference for precise unconstrained placement.
- Committed external points and analytical curves participate in that same inference language without becoming editable sketch entities. Visible earlier-sketch references and uniquely resolved coplanar model vertices, linear edges, circular edges, or bounded circular arcs may wake directly during Point or Line placement. A snap to an external point creates a local point plus explicit Coincident intent; alignment to that point creates a local point plus a non-dimensional horizontal or vertical point-pair constraint; a snap to an external line creates the applicable Midpoint, Point on line, or bounded Intersection relation; and a quadrant snap to an external circle or bounded arc creates Point on curve plus center alignment without materializing the same curve reference twice. Model wake-up persists a stable topology reference rather than display coordinates or renderer identity. Ambiguous topology, non-coplanar circles and arcs, and elliptical projections fail closed and remain available only through explicit **Use**. The preview glyph and source highlight appear before acceptance, and `Shift` suppresses the proposal for the current pointer sample.
