...
- The accessible task panel exposes sketch properties, constraints, dimensions, profiles, and the compact Finish and Cancel lifecycle actions. Geometry tools, local history, and profile features remain keyboard-accessible through the contextual toolbar and command palette. Pointer-free coordinate entry and canvas placement remain documented alpha accessibility limitations.
- The sketch task header keeps one compact, tooltip-labeled Cancel/Finish pair beside the sketch identity. The scrollable property area has no duplicate lifecycle footer. Extrude and Revolve share one remembered split-family control in the Sketch toolbar and remain available through the command palette; they do not appear as duplicate full-width task-panel actions. When closed profiles exist, launching either command from an open sketch captures every active-sketch selector in canonical order, persists the draft first, and opens the ordinary feature task with that complete editable selection. Finishing the sketch first remains an equivalent path.
