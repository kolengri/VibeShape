...
| Task panel | Active command parameters, properties, contextual help, diagnostics | Global navigation |
...
| Drag point | Keeps the latest pointer position in viewport-local preview state, renders only the affected point and curves over the last valid exact solution, defers live solving until a brief pointer pause, coalesces solving to one in-flight request plus the latest pending target, forwards the previous continuation state, and publishes one global draft plus one local undo checkpoint only when the gesture ends |
| Modify line | Trim, Extend, and Split target the line under the pointer, show a crosshair cursor, and commit one successful analytical operation as one local undo checkpoint |
...
- Direct modification tools expose only supported curve kinds as pointer targets. Line Trim uses the neighboring bounded line intersections around the click, Extend uses the clicked-near endpoint and nearest reachable bounded line, and Split projects the click onto the line interior. The retained line keeps its stable identity; detached endpoint-only constraints are removed, while points and constraints still owned by adjacent geometry remain. Unsupported or degenerate actions leave the draft unchanged.
...
- Point-drag pointer samples are reduced to the latest sample per animation frame before inference and viewport-local React publication. Stationary geometry retains one indexed, memoized base presentation; a transient overlay redraws only the dragged point and curves incident to it. Continuous movement restarts a short live-solve idle delay, so parent-workspace rendering and worker requests do not run for every animation frame. A pause or pointer release reuses the unchanged schema-valid sketch and carries the latest drag target separately; release preserves the final overlay until the exact result settles and records exactly one draft edit. Full-record cloning, schema parsing, full-solution copying, and full inference-reference reconstruction belong outside the pointer hot path.
...
- Each project card shows name, preview when available, modification time, storage state, and an accessible action menu.
