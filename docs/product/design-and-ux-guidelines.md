...
- Point-drag pointer samples are reduced to the latest sample per animation frame before inference and React publication. Stationary geometry retains one indexed, memoized base presentation; a transient overlay redraws only the dragged point and curves incident to it. Solver requests remain coalesced, and pointer release still records exactly one draft edit. The per-frame solver request reuses the unchanged schema-valid sketch and carries the drag target separately; full-record cloning, schema parsing, full-solution copying, and full inference-reference reconstruction belong outside the pointer hot path.
