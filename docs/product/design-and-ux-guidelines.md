...
| Modify curve | Trim and Split target a line, arc, or circle under the pointer; Extend targets a line or arc; every mode shows a crosshair cursor and commits one successful analytical operation as one local undo checkpoint |
...
- Point-drag client-coordinate samples are reduced to the latest sample per animation frame before layout measurement, sketch-coordinate conversion, inference, and viewport-local React publication. Stationary geometry retains one indexed, memoized base presentation; drag start isolates its affected curves before the first movement frame, and a transient overlay redraws only the dragged point and incident curves. Continuous movement restarts a short live-solve idle delay, so parent-workspace rendering and worker requests do not run for every animation frame. A pause or pointer release reuses the unchanged schema-valid sketch and carries the latest drag target separately; release preserves the final overlay until the exact result settles and records exactly one draft edit. Full-record cloning, schema parsing, full-solution copying, full inference-reference reconstruction, and per-raw-event layout reads belong outside the pointer hot path.
