...
- The sketch task header keeps one compact, tooltip-labeled Cancel/Finish pair beside the sketch identity. The scrollable property area has no duplicate lifecycle footer. Extrude and Revolve share one remembered split-family control in the Sketch toolbar and remain available through the command palette; they do not appear as duplicate full-width task-panel actions. When a closed profile exists, launching either command from an open sketch persists the draft first and carries its stable selector into the ordinary feature task. Finishing the sketch first remains an equivalent path.
