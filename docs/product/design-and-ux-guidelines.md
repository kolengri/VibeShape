...
- The accessible task panel exposes sketch properties plus the compact Finish, Cancel, and diagnostic-tools actions. The diagnostic action opens the optional Constraint manager for filtered constraint and dimension inspection; the manager is otherwise absent from the primary panel. Solver conflicts remain visible in the canvas and solve summary before it is opened, while canvas annotations and contextual actions remain primary. Closed profiles are selected and highlighted directly in the canvas instead of being duplicated as numbered form buttons. Geometry tools, local history, and profile features remain keyboard-accessible through the contextual toolbar and command palette. Pointer-free coordinate entry and canvas placement remain documented alpha accessibility limitations.
