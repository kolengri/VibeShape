...
- Replacing a model-backed external reference is an explicit graphical repair workflow, not a form select. Its icon-only row action enters a pressed Replace state with a localized tooltip and accessible action name; the viewport then offers only compatible topology from the same producing feature. One pick replaces only the stable topology target, preserves the external-reference ID, projected entity IDs, metadata, and dependent constraints, and exits to Select. Cancel leaves the previous target intact. Automatic or ambiguous rebinding is prohibited.
