...
- Point and line placement use deterministic screen-tolerance inference. Existing-point snaps reuse the stable point identity; midpoint, bounded segment-intersection, point-on-line, distant point-to-point horizontal or vertical alignment, exact circle or bounded-arc quadrants, exact full-ellipse axis quadrants, and exact generic full-ellipse perimeter snaps persist the corresponding relations; and line direction candidates persist horizontal, vertical, parallel, perpendicular, or endpoint-tangent intent. A round-curve quadrant preserves Point on curve plus horizontal or vertical alignment to the stable center. A full-ellipse Quadrant preserves the chosen primary or secondary axis and positive or negative side through later axis edits, while a perimeter snap preserves an exact Point on ellipse locus without reducing the ellipse to a circle or display polyline. Point dragging accepts the same point-relation candidates in its single release commit. Holding `Shift` suppresses inference for precise unconstrained placement.
- Committed external points and analytical curves participate in that same inference language without becoming editable sketch entities. Visible earlier-sketch references and uniquely resolved coplanar model vertices, linear edges, circular edges, bounded circular arcs, or full ellipses may wake directly during Point or Line placement. A snap to an external point creates a local point plus explicit Coincident intent; alignment to that point creates a local point plus a non-dimensional horizontal or vertical point-pair constraint; a snap to an external line creates the applicable Midpoint, Point on line, or bounded Intersection relation; a quadrant snap to an external circle or bounded arc creates Point on curve plus center alignment; and a full-ellipse snap creates either the dedicated axis-and-side Quadrant constraint or one exact Point on ellipse relation without materializing the same curve reference twice. A uniquely resolved coplanar model circle projected analytically as an ellipse follows the same full-ellipse path. Model wake-up persists a stable topology reference rather than display coordinates or renderer identity. Ambiguous topology, non-coplanar curves, elliptical arcs, and unsupported curved topology fail closed and remain available only through explicit **Use**. The preview glyph and source highlight appear before acceptance, and `Shift` suppresses the proposal for the current pointer sample.
