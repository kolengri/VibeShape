...
- Entering sketch edit keeps the existing Three.js viewport mounted so ordinary Model/Sketch transitions do not reset its camera or dispose its scene. Until shared camera and picking are implemented, that scene is visible as a passive context layer: it exposes no overlapping controls and cannot capture pointer input from the transparent analytical sketch surface above it.
