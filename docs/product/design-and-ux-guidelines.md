...
- The accessible task panel exposes sketch properties and an optional collapsed Constraint manager for diagnostic constraint and dimension editing, plus the compact Finish and Cancel lifecycle actions. The manager opens explicitly; solver conflicts remain visible in the canvas and solve summary before it is opened, while canvas annotations and contextual actions remain primary. Closed profiles are selected and highlighted directly in the canvas instead of being duplicated as numbered form buttons. Geometry tools, local history, and profile features remain keyboard-accessible through the contextual toolbar and command palette. Pointer-free coordinate entry and canvas placement remain documented alpha accessibility limitations.
