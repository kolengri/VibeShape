...
- Entering sketch edit keeps the existing Three.js viewport mounted so ordinary Model/Sketch transitions do not reset its camera or dispose its scene. Normal mode aligns that existing orthographic camera to the exact resolved support frame, keeps model and reference context visible below the transparent analytical sketch surface, and gives pointer ownership to 2D editing. The viewer hides the active sketch's committed display and every transitive dependent sketch or feature for the duration of the edit, preventing stale geometry from appearing beside the draft while preserving its support and independent model context. Terminal-body presentation is recomputed after that transient rollback, so the latest upstream body state replaces a hidden consuming feature; an ordinary user Hide action never reveals historical geometry. **Orbit 3D view** MUST keep the analytical editor mounted but visually hidden and inert, give the viewer camera-only pointer ownership, suppress model picking and selection callbacks, and show exactly one current unsaved solved sketch as derived world-space display geometry. **Normal to sketch** MUST restore support alignment without changing the authored draft, selected entities, selected profile, local undo/redo, or document state. These view commands use distinct icon-only actions with localized tooltips and active state; three identical visibility icons are not an acceptable substitute. Graphical external-reference picking remains a separate selection-broker capability and MUST NOT be inferred from camera motion.
