...
- Activating Use external geometry in the normal sketch view shows a persistent selection instruction. Hover and keyboard focus must pair the amber geometry preview with an immediate visible label for the exact sketch or model candidate; the label is presentation only and never becomes reference identity. Leaving the target clears the label without changing the draft, and overlapping targets continue through the bounded labeled chooser.
