...
- Stable high-frequency actions in the application bar and command toolbar use icon-only controls instead of repeating the same label beside the icon. A task-panel command header may use the established compact Cancel and Accept icons when both have localized accessible names and tooltips. Parameter actions, destructive confirmation, menus, and unfamiliar one-off actions retain visible text when the label carries essential meaning.
...
- Extrude and Revolve source references use compact sketch-icon/name links with a visual connector instead of prose such as “Profile from Sketch 1”. Hover or keyboard focus highlights the exact consumed region and its source History row while idle. Multi-profile inputs expose each region separately. A hidden source sketch is revealed only for the preview and returns to its prior visibility afterward; stale selectors fail closed. Click locates and focuses the source row without opening edit, and Escape returns focus to the consuming feature. Full source identity and dependency details remain accessible in tooltips. Active tools retain their own picking authority and offer locate-only links.
...
| Extrude or Revolve from the Sketch toolbar | Selects every closed profile in the active sketch, saves the sketch through the same single-flight revision-checked add or update, and opens the feature only after persistence succeeds; the ordinary feature task then allows individual profiles to be removed or restored, while failure preserves the complete draft and reports the save error |
...
- Point and line placement use deterministic screen-tolerance inference. Existing-point snaps reuse the stable point identity; midpoint, bounded segment-intersection, point-on-line, distant point-to-point horizontal or vertical alignment, exact circle or bounded-arc quadrants, exact full-ellipse axis quadrants, exact generic full-ellipse perimeter, and exact positive-sweep elliptical-arc perimeter snaps persist the corresponding relations; and line direction candidates persist horizontal, vertical, parallel, perpendicular, or endpoint-tangent intent. A round-curve quadrant preserves Point on curve plus horizontal or vertical alignment to the stable center. A full-ellipse Quadrant preserves the chosen primary or secondary axis and positive or negative side through later axis edits, while ellipse perimeter snaps preserve exact full or bounded loci without reducing either curve to a circle or display polyline. Point dragging accepts the same point-relation candidates in its single release commit. Holding `Shift` suppresses inference for precise unconstrained placement.
- Committed external points and analytical curves participate in that same inference language without becoming editable sketch entities. Visible earlier-sketch points, lines, circles, bounded circular arcs, full ellipses, and elliptical arcs plus uniquely resolved coplanar model vertices, linear edges, circles, and bounded circular arcs may wake directly during Point or Line placement. A snap to an external point creates a local point plus explicit Coincident intent; alignment to that point creates a local point plus a non-dimensional horizontal or vertical point-pair constraint; a snap to an external line creates the applicable Midpoint, Point on line, or bounded Intersection relation; a quadrant snap to an external circle or bounded arc creates Point on curve plus center alignment; a full-ellipse snap creates either the dedicated axis-and-side Quadrant constraint or one exact Point on ellipse relation; and an elliptical-arc snap creates one exact positive-sweep bounded relation without materializing the same curve reference twice. A uniquely resolved model circle or circular arc projected analytically as an ellipse or elliptical arc follows the corresponding exact path. Model wake-up persists a stable topology reference rather than display coordinates or renderer identity. Ambiguous topology, non-coplanar curves, and unsupported curved topology fail closed and remain available only through explicit **Use**. The preview glyph and source highlight appear before acceptance, and `Shift` suppresses the proposal for the current pointer sample.
...
- Constraint actions are enabled from compatible selections only. A point plus a full ellipse exposes the same **Point on curve** action used for round curves, but persists the exact ellipse-locus relation; an elliptical arc does not offer that action until bounded-curve semantics exist. Repeating the same semantic constraint is idempotent.
...
- The accessible task panel exposes sketch properties plus the compact Finish and Cancel actions. A dedicated diagnostic action in the contextual Sketch toolbar opens the optional Constraint manager for filtered constraint and dimension inspection; the manager is otherwise absent from the primary panel. Solver conflicts remain visible in the canvas and solve summary before it is opened, while canvas annotations and contextual actions remain primary. Closed profiles are selected and highlighted directly in the canvas instead of being duplicated as numbered form buttons. Geometry tools, local history, diagnostics, and profile features remain keyboard-accessible through the contextual toolbar and command palette. Pointer-free coordinate entry and canvas placement remain documented alpha accessibility limitations.
- The sketch task header keeps one compact, tooltip-labeled Cancel/Finish pair beside the sketch identity. The scrollable property area has no duplicate lifecycle footer. Extrude and Revolve are separate direct icon commands in the Sketch toolbar and remain available through the command palette; they do not hide behind a remembered family menu or appear as duplicate full-width task-panel actions. When closed profiles exist, launching either command from an open sketch captures every active-sketch selector in canonical order, persists the draft first, and opens the ordinary feature task with that complete editable selection. Finishing the sketch first remains an equivalent path.
- Related geometry variants use one split family action. The primary icon invokes the active or last-used variant; the adjacent Radix menu exposes every variant through keyboard navigation and a localized accessible name. Distinct modeling operations such as Extrude and Revolve never share this treatment.
- While a sketch is active, the contextual toolbar omits workspace switches and model-authoring commands that cannot participate in the current sketch. It orders direct feature transitions first, then core selection and reference tools, geometry families, Dimension and Construction, direct modification, view controls, diagnostics, and local history. Dimension and Trim remain visible at compact desktop widths; lower-frequency modifications move into one labeled overflow menu. Family membership, default variants, and the `S` shortcut surface are declared by the shared command descriptors rather than duplicated in shell components.
...
- Entering sketch edit keeps the existing Three.js viewport mounted so ordinary Model/Sketch transitions do not reset its camera or dispose its scene. Normal mode aligns that existing orthographic camera to the exact resolved support frame, keeps model and reference context visible below the transparent analytical sketch surface, and gives pointer ownership to 2D editing. Each eligible earlier sketch has exactly one rendering owner in this mode: the analytical SVG layer draws projected ordinary geometry as fine dashed strokes with the contrast-safe `--sketch-reference-context` token and construction geometry as reduced-emphasis long dashed strokes, while the Three.js layer omits that same saved-sketch display. Active authored geometry remains solid. The solve summary names every visible source sketch and its projected entity count so reference context is discoverable before **Use** is activated. This prevents doubled circles, points, and other curves when the two layers are aligned and prevents valid reference context from being mistaken for duplicate authored geometry. The viewport rolls back at the active History item: it hides the active sketch's committed display and every later sketch or feature, including independent later items, while preserving earlier support and model context. The History marker must describe later items as rolled back and may direct users to **Show final result**; it must not imply that downstream context is already visible. Terminal-body presentation is recomputed after that transient rollback, so the latest upstream body state replaces a hidden consuming feature; an ordinary user Hide action never reveals historical geometry. **Show final result** may reveal the downstream result as explicitly labeled, display-only context without making any rolled-back item selectable, inferable, or eligible for **Use**. Turning it off restores rollback presentation immediately. **Orbit 3D view** keeps ordinary sketch editing active at the chosen camera angle. The viewer publishes an orthographic projection of the resolved support plane; the analytical editor uses that same transform for rendering, pointer placement, dragging, and annotation anchors. Primary input edits the sketch, middle drag orbits, secondary drag pans, and wheel zooms toward the pointer. Earlier sketches retain their world-space Three.js context while the active sketch has exactly one analytical rendering owner. Near-edge-on planes reject placement with visible guidance to rotate or choose Normal to sketch. Ordinary model picking remains suppressed. Use, Pierce, and Intersection explicitly transfer pointer ownership to the 3D selection broker and keep the analytical editor inert only while those selection tools are active. **Normal to sketch** MUST restore analytical ownership and support alignment without changing the authored draft, selected entities, selected profile, local undo/redo, final-context preference, or document state. These view commands use distinct icon-only actions with localized tooltips and active state; three identical visibility icons are not an acceptable substitute. Graphical external-reference picking remains a separate selection-broker capability and MUST NOT be inferred from camera motion. During **Use**, hovering overlapping orbit geometry MUST expose a bounded closest-first Select Other stack instead of silently committing the first ray hit. The compact overlay previews each candidate in the scene and supports grave accent, Shift+grave accent, Enter, Escape, pointer hover, and pointer activation without persisting renderer-local candidate IDs.
- Accepting an external model reference MUST keep the same localized feature-and-subshape label in hover, Select Other, the reference list, and a reopened sketch. Presentation ordinals use canonical topology identity ordering rather than evaluation-local worker traversal, so an unchanged semantic subshape keeps its label when candidate arrays are reordered. The list resolves stable topology intent against the current rebuild, reports missing or ambiguous geometry explicitly, and never exposes a semantic role, lineage token, transient candidate ID, or raw feature UUID as product copy.
- Replacing a model-backed external reference is an explicit graphical repair workflow, not a form select. Its icon-only row action enters a pressed Replace state with a localized tooltip and accessible action name. A live reference offers only compatible topology from the same producing feature; a reference intentionally orphaned by source-feature deletion offers compatible topology from visible replacement features and becomes live only after an explicit pick. One pick replaces only the stable topology target, preserves the external-reference ID, projected entity IDs, metadata, and dependent constraints, and exits to Select. Cancel leaves the previous target intact. Automatic or ambiguous rebinding is prohibited.
...
- A supported planar model face may replace the origin plane as sketch support. If the face is already selected, `Create sketch` enters editing immediately; during support selection the 3D viewport accepts either a visible origin plane or an eligible face. A model face under the pointer takes precedence over an overlapping origin datum, so origin-plane helpers never block sketching from an extrusion cap or other eligible face. Hover identifies the exact target as `Feature · Face N`. When two or more eligible model faces overlap at the pointer, primary click opens a bounded, non-modal Select Other list instead of silently accepting the first ray hit. Pointer hover previews one exact face; grave accent and Shift+grave accent cycle; Enter accepts; Escape dismisses and returns focus to the viewport. The chosen label remains identical in the support control after Finish and reopen, while stable topology intent—not the presentation ordinal or renderer face ID—remains the persisted identity. The task panel does not expose a misleading editable origin-plane choice. Unsupported faces preserve selection and require a different support rather than silently falling back.
- An open sketch exposes an icon-only, tooltip-labeled Replace Support action beside its current support. It opens the same graphical 3D picker used at creation, shows replacement-specific guidance, and offers origin-plane buttons only as the keyboard equivalent. Cancel does not mutate the draft. Accepting an earlier origin plane, datum plane, or supported planar face preserves all authored identities, creates one local undo checkpoint, and returns directly to sketch editing. The picker hides the active sketch and every later History item; forward or cyclic support is never selectable or silently accepted.
- Activating Intersect planar face from the normal sketch view transitions directly to the orbitable 3D face picker with explicit target guidance. The command disables final-result context while selecting, so the existing History rollback remains authoritative and only compatible earlier planar faces may commit an associative intersection. Returning to Normal to sketch disarms Intersection back to Select; never leave a pressed Intersection command over a normal analytical canvas that cannot select faces.
- Earlier rollback-compatible sketch geometry and projected model topology remain graphically available in the normal sketch view. In Select, hover and keyboard focus pair the amber geometry preview with an immediate visible source label, and primary activation creates the same stable read-only reference as Use external geometry. A selected local point may consume a compatible external point in the same activation. Use remains the explicit persistent projection mode and the entry point for repair. Leaving a target clears the label without changing the draft. Overlapping targets open the bounded labeled chooser; grave accent and Shift+grave accent cycle its active candidate forward and backward, update the analytical preselection, and Enter accepts the highlighted source. Presentation labels and renderer-local candidate IDs never become reference identity.
...
- Every committed sketch row exposes independent icon-only, tooltip-labeled visibility and Delete actions. Delete opens an explicit confirmation and is unavailable for the currently edited sketch, read-only documents, an extrusion-owned sketch, or a source sketch with incoming external references. A visible saved sketch renders its solved curves and points in the model viewport on the exact origin, planar-face, extrusion-cap, or Datum Plane support frame. Construction geometry uses a distinct dashed, reduced-emphasis treatment. Closed saved regions use a subtle derived fill; hover names and highlights the exact `Sketch · Profile N`, and primary click selects its canonical boundary selector without entering sketch edit. Hiding the sketch removes both display and hit regions and clears an invalid profile selection. The accessible profile controls remain the keyboard-equivalent surface. Sketch lines, points, profile samples, support-frame copies, and viewer triangles are disposable worker/viewer output, never semantic document content or topology identity. Finishing a sketch returns to the model viewport with that sketch visible and its selected closed profile retained for Extrude; activating the sketch row re-enters editing directly.

- Ordered sketch-reference chains remain canvas-first. A later sketch may select projected point, line, or curve geometry owned by an intermediate sketch. The intermediate sketch remains the direct labeled owner, every persisted reference uses stable projected identities, and a compatible edit to the original source recursively updates downstream projections without replacing those identities.
...
- A supported feature parameter that has a clear spatial meaning SHOULD expose direct viewport manipulation synchronized with its exact task-panel field. Extrude depth uses one support-normal handle; symmetric depth displays half the authored total on each side. Revolve angle uses a curved arrow with visible start and end radial lines around the selected analytical axis. The manipulator is disposable preview state, preserves the camera during preview rebuilds, replaces a changed expression with a literal in the active project unit, and never commits a revision until Apply.
- Previewed feature forms MUST invalidate the previous exact-preview acknowledgement as soon as any draft value changes. Create, Apply, and Update remain disabled while the latest candidate is idle, loading, invalid, or failed and become available only after the isolated document-worker preview reports ready. A failed preview preserves both the authored form buffer and last committed viewport geometry.
...
- Tooltips supplement accessible names; they never contain required instructions, errors, or interactive content, and their transient layers never intercept pointer input intended for the viewport or adjacent commands.
