...
- Committed external points and lines participate in that same inference language without becoming editable sketch entities. Visible earlier-sketch references and exact coplanar model vertices or linear edges may wake directly during Point or Line placement. A snap to an external point creates a local point plus explicit Coincident intent; a snap to an external line creates the applicable Midpoint, Point on line, or bounded Intersection relations. Model wake-up persists a stable topology reference rather than display coordinates or renderer identity. The preview glyph and source highlight appear before acceptance, and `Shift` suppresses the proposal for the current pointer sample.
