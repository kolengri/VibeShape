...
| Dimension | `D` activates a persistent dimension mode; one click selects a line or round curve, sequential clicks collect two points or two lines without requiring a modifier, and a compatible selection focuses the exact variable-aware driving-value field; points remain selectable but cannot be dragged in this mode |
