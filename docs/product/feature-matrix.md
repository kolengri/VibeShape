...
Extrude and Revolve can use a bounded set of profiles from one solved sketch. A normal click replaces the selection and `Shift`-click toggles another region. One-profile records preserve the existing operation contracts; multi-profile records are versioned separately, permit `New` only, solve the source sketch once, and build every selected analytical region transactionally. Their result may contain from one through the selected-profile count of positive-volume solids. Multi-profile results remain excluded from downstream Add, Remove, Intersect, Boolean, and Subtract target selection until stable per-solid result identity and merge scope are defined.
