...
Extrude and Revolve can use a bounded set of profiles from one solved sketch. A normal click replaces the selection and `Shift`-click toggles another region. One-profile records preserve the existing operation contracts. Multi-profile New records are versioned separately, solve the source sketch once, build every selected analytical region transactionally, and may contain from one through the selected-profile count of positive-volume solids. Extrude also has a distinct target-first multi-profile Add/Remove/Intersect version: it combines the canonical tools before one Boolean operation and accepts only one valid positive-volume result solid, so that result remains an ordinary downstream target. Multi-profile New results remain excluded from downstream target selection until stable per-solid identity and merge scope are defined; modifying multi-profile Revolve remains open.
