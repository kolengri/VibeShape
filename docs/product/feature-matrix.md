...
The first sketch-driven operation implements exact new/add/remove/intersect extrusion from a stable `SketchProfileSelector`. The semantic feature stores the selector, a literal or `#variable` distance `Quantity`, symmetric state, and operation intent. `New` has no B-Rep dependency; every modifying operation stores exactly one explicit terminal-feature target dependency. Each rebuild solves the referenced sketch once, resolves the selector fail-closed, and passes bounded transient analytical line, arc, circle, and ellipse loops to geometry protocol v8; solved coordinates and profile indices are never persisted. OCCT creates one exact prism, then applies Fuse, Cut, or Common for a modifying operation. Every result must be one valid positive-volume solid. The product offers every solver-produced region for selection by stable boundary identity, including profiles with holes, and supports operation and target editing, variable-driven rebuild, legacy extrusion migration, reload, and dependency-safe deletion. A debounced schema-valid feature draft rebuilds in an isolated document-worker session through the same OCCT path; changed terminal bodies render as a translucent, non-selectable preview, invalid previews leave committed geometry unchanged, and Apply still creates one ordinary command. Multi-profile feature input and downstream `TopoRef` authoring remain open. See [ADR-0019](../adr/0019-selector-backed-new-body-extrusion.md) and [ADR-0023](../adr/0023-explicit-target-extrusion-operations.md).
