...
Extrude and Revolve can use a bounded set of profiles from one solved sketch. A normal click replaces the selection and `Shift`-click toggles another region. One-profile records preserve the existing operation contracts. Multi-profile New records are versioned separately, solve the source sketch once, build every selected analytical region transactionally, and may contain from one through the selected-profile count of positive-volume solids. Distinct target-first multi-profile Add/Remove/Intersect versions combine the canonical tools before one Boolean operation and accept only one valid positive-volume result solid, so each modifying result remains an ordinary downstream target. Revolve resolves one shared exact origin, sketch-line, or model-edge axis and orders a distinct axis source after target and support dependencies. Multi-profile New results remain excluded from downstream target selection until stable per-solid identity and merge scope are defined.

Blind Extrude depth is directly editable through one 3D handle aligned to the selected support frame normal. The handle and exact project-unit-aware distance field remain bidirectionally synchronized during disposable create and edit previews; symmetric mode presents half of the authored total distance on each side. Drag cancellation restores the gesture start, and only the ordinary Apply path persists the resulting feature record.
...
The first sketch-driven operation implements exact new/add/remove/intersect extrusion from one stable `SketchProfileSelector` or a bounded same-sketch selector set. The semantic feature stores stable selectors, a literal or `#variable` distance `Quantity`, symmetric state, and operation intent. An origin plane, a stable planar-face `TopoRef`, or a first-class signed offset Datum Plane supplies the sketch frame. The initial model-face support set covers every planar Box face, Cylinder cap, recursively resolved Extrusion cap, and `datum.plane`; unsupported or missing topology fails closed. `New` has no B-Rep body dependency but may have support evaluation dependencies, while every modifying operation stores exactly one explicit terminal-feature target plus any distinct support dependency. Each rebuild solves the referenced sketch once, resolves every selector and the support fail-closed, and passes bounded transient analytical curves plus an exact right-handed frame to geometry protocol v12. The same protocol publishes rebuild-local exact vertex, linear-edge, circle-edge, arc-edge, ellipse-edge, and elliptical-arc-edge geometry for fail-closed model references without persisting transient topology IDs. The sketcher exposes those records graphically in normal and orbit views and persists only stable `TopoRef` intent. OCCT creates one or more exact prisms, combines a multi-profile tool canonically, then applies Fuse, Cut, or Common once for a modifying operation. The product supports operation and target editing, variable-driven rebuild, preview, reload, and dependency-safe deletion. Datum Plane features accept an origin plane or selected supported face, retain signed literal or `#variable` offsets, rebuild dependent sketches, render as selectable translucent reference geometry with independent visibility, and remain excluded from body ownership, project thumbnails, and STEP/STL/3MF export. Cross-sketch profile sets, spline and other non-analytical curved model references, additional datum construction modes, and general topology repair remain open. See [ADR-0019](../adr/0019-selector-backed-new-body-extrusion.md), [ADR-0023](../adr/0023-explicit-target-extrusion-operations.md), [ADR-0024](../adr/0024-stable-planar-face-sketch-support.md), [ADR-0025](../adr/0025-first-class-offset-datum-planes.md), [ADR-0034](../adr/0034-versioned-multi-profile-feature-input.md), and [ADR-0036](../adr/0036-multi-profile-modifying-extrusion.md).
...
- P1: adjacent History reorder with DAG validation, stable item identity, single-flight persistence, undo/redo, and reload retention (implemented).
