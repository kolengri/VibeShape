...
The first sketch-driven operation implements exact new/add/remove/intersect extrusion from a stable `SketchProfileSelector`. The semantic feature stores the selector, a literal or `#variable` distance `Quantity`, symmetric state, and operation intent. An origin plane or a stable planar-face `TopoRef` supplies the sketch frame. The initial model-face support set covers every planar Box face, Cylinder caps, and recursively resolved Extrusion caps; unsupported or missing topology fails closed. `New` has no B-Rep body dependency but may have one support evaluation dependency, while every modifying operation stores exactly one explicit terminal-feature target plus any distinct support dependency. Each rebuild solves the referenced sketch once, resolves the selector and support fail-closed, and passes bounded transient analytical line, arc, circle, and ellipse loops plus an exact right-handed frame to geometry protocol v9; solved coordinates, profile indices, tessellation face IDs, and OCCT indices are never persisted. OCCT creates one exact prism, then applies Fuse, Cut, or Common for a modifying operation. Every result must be one valid positive-volume solid. The product offers every solver-produced region for selection by stable boundary identity, including profiles with holes, and supports operation and target editing, variable-driven rebuild, legacy extrusion migration, reload, and dependency-safe deletion. A debounced schema-valid feature draft rebuilds in an isolated document-worker session through the same OCCT path; changed terminal bodies render as a translucent, non-selectable preview, invalid previews leave committed geometry unchanged, and Apply still creates one ordinary command. Multi-profile feature input, semantic datum planes, and general topology repair remain open. See [ADR-0019](../adr/0019-selector-backed-new-body-extrusion.md), [ADR-0023](../adr/0023-explicit-target-extrusion-operations.md), and [ADR-0024](../adr/0024-stable-planar-face-sketch-support.md).
