...
| External selection | Use external geometry selects earlier sketch points, lines, and analytical curves plus visible committed model vertices, linear edges, circles, circular arcs, ellipses, and elliptical arcs in 2D or the persistent 3D viewport with source labels. Passive, selectable, and committed layers are mutually exclusive, renderer-local candidate IDs never persist, and bounded graphical choosers disambiguate overlapping normal and orbit-view candidates. Orbit candidates are ordered closest-first; grave accent cycles forward, Shift+grave accent cycles backward, Enter accepts, and Escape closes the chooser. | Add spline and other non-analytical feature-topology candidates plus source filters. | P0 |
| Intersection and Pierce | Intersection selects one visible planar model face in orbit mode and creates one bounded associative linear OCCT section. Selection-first Pierce constrains one authored point to the exact transverse crossing of one visible earlier-sketch line and the active support. Both fail closed for unsupported geometry. | Add curved surfaces, multiple analytical result segments, curved/model-edge Pierce, and broader graphical repair. | P0 |
| External constraints | Projected geometry supports Coincident, Horizontal/Vertical, Point on line/curve/ellipse/elliptical arc, Parallel, Perpendicular, Equal, Tangent, Concentric, compatible dimensions, and the explicit line Pierce relation where mathematically valid. | Add remaining curve measurements and spline/general-surface relations. | P0/P1 |
...
| Repair | Missing sketch-backed point, line, and analytical-curve sources remain as persisted broken intent. Exact current-generation worker evidence also detects model-backed topology, projection, and intersection failures. History identifies affected dependent sketches, distinguishes direct and chained failures, and opens the selected dependent directly for graphical repair. The external-reference row names the missing kind and replaces it from compatible earlier-sketch geometry without changing projected identities or local constraints. Compatible model-backed references also support graphical same-feature repair. | Add source-owned ambiguous diagnostics, cross-highlighting, and multi-link traversal. | P0 |
...
The concrete persistence repository also exposes migrated recovery without changing the existing version-0
...
the same stable reference used by the normal-to-sketch drawing and task panel. Normal mode also renders every visible earlier analytical sketch curve as muted context. In Select, the existing graphical broker overlays exact hit targets, immediate source labels, keyboard focus, and bounded overlap choice; primary activation materializes one stable read-only reference without requiring a preliminary toolbar mode. Use remains available for repeated projection and repair. Center points use a distinct square marker rather than appearing as duplicate circles. Cross-support point and line references are transformed through exact source and target frames
...
- Stable vertex, linear-edge, circular-edge, and elliptical-edge selection from feature topology candidates is implemented.
- Exact circle, bounded circular-arc, full-ellipse, and bounded elliptical-arc projection into resolved target sketch frames is implemented.
- Source-frame/target-frame projection is deterministic; broader caching and guided repair diagnostics remain open.
...
- Same-sketch multi-profile selection is implemented for Extrude and Revolve. Both support target-first multi-profile Add, Remove, and Intersect with a one-solid result invariant; Revolve retains one shared exact axis. Explicit per-solid merge scope for multi-profile New remains open.
