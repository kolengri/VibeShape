...
|---|---|---|
| Onshape | A stable graphics area, feature list, contextual sketch toolbar, compact Accept/Cancel lifecycle, direct canvas constraints, secondary diagnostic Constraint manager, and direct Extrude/Revolve that accepts the open sketch first | Keep the viewport and model tree stable; keep only X/Finish in the sketch task header; keep diagnostics and profile features in the contextual toolbar; create and edit constraints primarily on the canvas |
| Autodesk Fusion | Contextual Sketch tab, persistent Browser, explicit Finish Sketch action, canvas selection, chronological parametric history, and a Sketch Palette for display and snapping preferences | Treat Sketch as a bounded command environment with a visible exit and one semantic commit; adopt a compact Sketch Options surface for view preferences without introducing a second modeling mode |
...
- Existing-point, segment-midpoint, positive-sweep arc-midpoint, bounded segment-intersection, point-on-line, point-to-point horizontal/vertical alignment, exact circle or bounded-arc quadrant, exact full-ellipse axis quadrant and perimeter, exact positive-sweep elliptical-arc perimeter, line direction, parallel, perpendicular, and endpoint-tangent candidates now provide inference guides and accepted persistent constraints. Alignment, round quadrant, arc-midpoint, and full-ellipse axis, side, or generic locus intent can target authored geometry or passively woken stable external points and curves without activating Use. Dragging an existing authored point uses the same visible earlier-sketch and uniquely resolved coplanar model candidates, materializes the stable reference only on release, and preserves `Shift` suppression. Non-coplanar and non-analytical projected geometry remains open.
- The [mandatory sketch precision toolset](sketch-toolset.md) is implemented for geometry, 14 geometric constraints, eight driving dimensions, and reference distance, projected-distance, angle, radius, and diameter measurements. The registered Dimension command is available from the toolbar, palette, and `D`; it collects compatible canvas selections without a modifier and opens the canvas editor at the placed annotation. Driving dimensions retain variable-aware expressions. Reference dimensions are value-less, add no solver equation, render a live parenthesized and dashed measurement, and remain stable after Finish and reopen. Selection-driven icon tools still use the complete selection, applied canvas glyphs are selectable, and only a driving dimension label opens the expression editor.
- Add numeric point placement, coordinate editing, ellipse-axis reference measurements, and explicit driving/reference conversion.
- Center Rectangle, Aligned Rectangle, Centered Aligned Rectangle, Midpoint Line, Three-point Circle, Center-point Ellipse, Elliptical Arc, both 3–50-side regular Polygon variants, Three-point Arc, and Tangent Arc are implemented with exact previews, persistent intent, one-step local history, and grouped toolbar/palette presentation. Center-point Ellipse uses three stable points, one solver-internal perpendicular axis equation, exact profile geometry, and exact OCCT extrusion. Elliptical Arc follows the center → primary axis → secondary radius/start → endpoint workflow, previews a temporary construction ellipse, and constrains both visible endpoints to the analytical ellipse through solver-owned trammels. Trim supports hover preselection, click targeting, and analytical drag-through across lines, circular arcs, circles, full ellipses, and elliptical arcs as one cancellable local gesture; Split supports the same curve family, while Extend supports open lines, circular arcs, and elliptical arcs. Ellipse operations currently use exact bounded line intersections. Linear and center-based Circular Pattern provide bounded variable-aware previews and materialize one undoable set of analytical copies. These exact domain operations preserve retained identity, repair detached endpoint dependencies, preview the two-point closed-circle split, and create one local history edit. Primary- and secondary-axis diameter dimensions are implemented for both ellipse entity types. Exact removable-span Trim preselection, round and ellipse boundaries for ellipse modification, spline authoring, associative patterns, direct circular-pattern center dragging, free-end Extend, and guided conflict repair remain open.
...
- Exact unsaved Extrude and Revolve previews include bounded same-sketch multi-profile selection, `Shift`-toggle interaction, removal/clear actions, and edit/reopen retention. Both features preview and persist target-first multi-profile Add, Remove, and Intersect with one canonical combined tool and one-solid result invariant. Revolve keeps one shared exact axis and an independently auditable axis-source dependency. Per-solid selection for multi-profile New remains open.
...
- World-axis translation manipulators are implemented for Box and Cylinder create/edit tasks and remain paired with exact X/Y/Z task-panel fields through disposable preview. Add primitive rotation, scale, snapping, and feature-specific manipulators only with equally precise field and command semantics.
