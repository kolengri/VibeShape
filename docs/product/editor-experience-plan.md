...
- Exact unsaved Extrude and Revolve previews include bounded same-sketch multi-profile New selection, `Shift`-toggle interaction, removal/clear actions, and edit/reopen retention. Extrude additionally previews and persists target-first multi-profile Add, Remove, and Intersect with one canonical combined tool and one-solid result invariant. Multi-profile modifying Revolve and per-solid selection for multi-profile New remain open.
