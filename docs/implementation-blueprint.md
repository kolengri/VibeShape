...
`E02` and `E10` now have one shared executable foundation slice: UUIDv7 domain identities, document create and rename commands, strict actor and event schemas, deterministic replay, revision-safe disposable drafts, a validated first-party document module with automation metadata, and a trusted dispatcher that enforces descriptor-handler parity before routing commands. This does not complete either epic: units, feature DAGs, queries, geometry validation and preview, persistence, undo/redo, cancellation, expiry, and confirmation policy remain open. The slice proves an adapter-neutral command path and does not justify an MCP workspace or SDK dependency yet.
