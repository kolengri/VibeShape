...
**Production follow-through:** the exact reviewed output is now promoted behind sketch schema v0 and document protocol v7. Revisioned sketch commands, semantic reference validation, variable-backed dimensions, stable-ID continuation, drag precedence, worker replacement/rebuild/retry, runtime hash verification, `.vshape` preservation, deterministic endpoint-connected profile extraction, stable boundary selectors, selector-backed exact new/add/remove/intersect extrusion, disposable create/edit solid preview through an isolated production worker, the initial 1,000-point browser budget, complete transient-draft solves, interactive Point/Line/corner Rectangle/center Rectangle/Circle/center-point Arc/three-point Arc authoring, P0 constraints and dimensions, profile-region selection, inference glyphs, and local sketch undo/redo are executable. Interior-intersection splitting, trim/extend, and multi-profile feature input remain the next sketch slice. See [ADR-0018](adr/0018-deterministic-sketch-profile-extraction.md), [ADR-0019](adr/0019-selector-backed-new-body-extrusion.md), [ADR-0021](adr/0021-transient-sketch-draft-solving.md), and [ADR-0023](adr/0023-explicit-target-extrusion-operations.md).
