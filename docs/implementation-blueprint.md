...
`E02` and `E10` now share executable foundation slices: UUIDv7 domain identities, document create and rename commands, strict actor and event schemas, deterministic replay, revision-safe disposable drafts, a validated first-party document module with command and query metadata, trusted dispatchers that enforce descriptor-handler parity, and a strict feature graph with deterministic evaluation scheduling and failure propagation. `@vibeshape/automation-api` supplies strict lifecycle schemas and the bounded `org.vibeshape.document.summary` schema version 1 read model. `@vibeshape/automation-host` adds host-generated owner-bound drafts, serialized operations, inactivity and count limits, bounded preview, idempotent discard, and atomic commit-port coordination. This does not complete either epic: units, feature commands/events, content hashing, geometry validation, durable persistence, undo/redo, cancellation, idempotent command replay, session pairing and revocation, and confirmation remain open. These slices prove adapter-neutral query, command, and feature-scheduling paths and do not justify an MCP workspace or SDK dependency yet.
