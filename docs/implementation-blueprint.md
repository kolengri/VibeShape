...
`E02` and `E10` now share executable foundation slices spanning strict UUIDv7 domain identities, dimensional expressions and document variables, revisioned commands and tamper-resistant events, trusted module and feature-type registries, canonical geometry identity, deterministic DAG scheduling, persisted drafts, worker-owned OCCT rebuild, and adapter-neutral automation contracts. The product path saves semantic revisions before rebuild, recovers or replaces failed workers, creates and edits explicitly positioned Box and Cylinder primitives, Boolean/Subtract, interactive analytical sketch, and selector-backed extrusion records, atomically refactors exact variable references, removes only dependency-safe leaf features, and exports terminal bodies as multi-object 3MF, exact STEP, or binary STL. The persistence boundary verifies checksummed history and writer leases; the `.vshape` v0 boundary now rereads and replays that complete history, writes a deterministic bounded archive, and atomically imports a fresh document only after structure, SHA-256, schema, and replay verification. Real-browser coverage proves variable-driven geometry and placement, recovery, editing, deletion, export, and native-project round-trip with authored formulas and feature sources intact. Persisted topology repair, persistent caches, autosave scheduling, native-format migrations and restore/copy policy, BroadcastChannel ownership UX, user-driven hard cancellation, extension-specific variable refactor contributions, richer expressions, committed document undo/redo, idempotent command replay, configurable print-quality export profiles and reports, interactive placement manipulators and primitive rotation, session pairing and revocation, and automation-host confirmation remain open. These slices prove adapter-neutral query, command, feature-schema, expression, identity, scheduling, persistence, portable-project, and dependent worker paths and do not justify an MCP workspace or SDK dependency yet.
