...
**Production follow-through:** the exact reviewed output is now promoted behind sketch schema v0 and document protocol v5. Revisioned sketch commands, semantic reference validation, variable-backed dimensions, stable-ID continuation, drag precedence, worker replacement/rebuild/retry, runtime hash verification, `.vshape` preservation, deterministic endpoint-connected profile extraction, stable boundary selectors, selector-backed exact new-body extrusion, and the initial 1,000-point browser budget are executable. Interior-intersection splitting, general profile picking, add/remove/intersect body semantics, and free-form product interaction remain the next sketch slice. See [ADR-0018](adr/0018-deterministic-sketch-profile-extraction.md) and [ADR-0019](adr/0019-selector-backed-new-body-extrusion.md).
...
`E02` and `E10` now share executable foundation slices spanning strict UUIDv7 domain identities, dimensional expressions and document variables, revisioned commands and tamper-resistant events, trusted module and feature-type registries, canonical geometry identity, deterministic DAG scheduling, persisted drafts, worker-owned OCCT rebuild, and adapter-neutral automation contracts. The product path saves semantic revisions before rebuild, recovers or replaces failed workers, creates and edits Box, Cylinder, and Boolean/Subtract features, atomically refactors exact variable references, removes only dependency-safe leaf features, and exports terminal bodies as STEP or binary STL. The persistence boundary verifies checksummed history and writer leases; the `.vshape` v0 boundary now rereads and replays that complete history, writes a deterministic bounded archive, and atomically imports a fresh document only after structure, SHA-256, schema, and replay verification. Real-browser coverage proves variable-driven geometry, recovery, editing, deletion, export, and native-project round-trip with authored formulas and feature sources intact. Persisted topology repair, persistent caches, autosave scheduling, native-format migrations and restore/copy policy, BroadcastChannel ownership UX, user-driven hard cancellation, extension-specific variable refactor contributions, richer expressions, undo/redo, idempotent command replay, print-quality export profiles and reports, production 3MF integration, session pairing and revocation, and automation-host confirmation remain open. These slices prove adapter-neutral query, command, feature-schema, expression, identity, scheduling, persistence, portable-project, and dependent worker paths and do not justify an MCP workspace or SDK dependency yet.
