...
`E02` and `E10` now share executable foundation slices: UUIDv7 domain identities, canonical length/angle/scalar quantities, document-variable and dimensional-expression schema v0, revisioned document/variable/feature commands including atomic stable-ID variable rename and dependency-safe leaf feature removal, strict actor and event schemas, deterministic replay, revision-safe disposable drafts, validated first-party document, feature-kernel, and part-design modules, trusted command and feature-type registries that enforce descriptor-handler and active-type parity, unit-aware box/cylinder and ordered Boolean/Subtract parameter schemas, registry-bound expression resolution and add/update preflight, bounded semantic content projections, canonical feature-content identity version `0`, and a strict feature graph with atomic mutation, deterministic asynchronous evaluation scheduling, and failure propagation. Content identity uses resolved parameter values, ordered dependency hashes, and slot-relative topology references; includes exact host/geometry/tolerance/provider identity; ignores record-only UUID/label/suppression, variable names, equivalent expression formatting, and source-unit presentation metadata; and validates an injected SHA-256 digest without exposing environment globals to the domain. `@vibeshape/application` accepts a runtime-validated committed document, evaluates its variable DAG, resolves trusted parameters, constructs its derived DAG, automatically derives dirty roots from canonical prior resolved feature fingerprints, and coordinates the scheduler with canonical hashing and a serializable geometry port. Its persisted session use case serializes mutations, renews single-writer leases, dispatches the ordinary command, atomically persists the event and snapshot, then rebuilds the saved revision; storage failure advances nothing, while infrastructure rebuild failure retains the committed semantic revision for retry. The same session exposes read-only export because file generation does not mutate semantic state or require a writer lease. `@vibeshape/document-worker` composes rebuild and exact-revision export with the geometry engine outside the main thread through document protocol v3, owns successful state per document, rejects stale queued generations, transfers cloned mesh and owned STEP/STL buffers, synchronizes native ownership to exact successful feature-content hashes, and disposes document-scoped native ownership. Its document session detects worker errors, message failures, timeouts, and retryable worker failures, then replaces the client, increments generation, rebuilds the latest successful semantic snapshot including variables, and retries once. `@vibeshape/automation-api` supplies strict lifecycle schemas and the bounded `org.vibeshape.document.summary` schema version 1 read model. `@vibeshape/automation-host` adds host-generated owner-bound drafts, serialized operations, inactivity and count limits, bounded preview, idempotent discard, and atomic commit-port coordination; validated variable and box feature-add fixtures use the same command path without exposing an MCP tool. The persistence boundary accepts replayable variable and feature revisions, checksum-recovers semantic history, and enforces writer lease epochs; structural replay, removal, and suppression do not require the contributing runtime. `E03` and `E04` intersect through geometry protocol v7: the engine validates and hashes resolved feature identities, binds dependency UUIDs to canonical hash slots without including them in identity, evaluates box, cylinder, and Boolean subtraction, emits typed diagnostics and transferable meshes, keeps exact document-scoped B-Rep ownership with transactional replacement, and exports successful terminal bodies as STEP or binary STL with a temporary compound for multiple roots. The web integration harness proves committed-document-to-OCCT coordination, variable-driven selective rebuild, clean reuse, hard worker replacement and semantic recovery, interrupted page-reload recovery from IndexedDB, clean save/reopen rebuild, persisted exact-token variable refactor, dependency-safe feature deletion, automatic descendant-only rebuild after recovery, multi-body STEP/STL downloads, health, and disposal across Chromium, Firefox, and WebKit. Persisted topology lineage and repair, persistent caches, autosave scheduling, `.vshape`, BroadcastChannel ownership UX, user-driven hard cancellation, extension-specific variable refactor contributions, richer expressions, undo/redo, idempotent command replay, print-quality export profiles and reports, production 3MF integration, session pairing and revocation, and automation-host confirmation remain open. These slices prove adapter-neutral query, command, feature-schema, expression, identity, scheduling, persistence, and dependent worker paths and do not justify an MCP workspace or SDK dependency yet.
