...
**Production follow-through:** the exact reviewed output is now promoted behind sketch schema v0 and document protocol v6. Revisioned sketch commands, semantic reference validation, variable-backed dimensions, stable-ID continuation, drag precedence, worker replacement/rebuild/retry, runtime hash verification, `.vshape` preservation, deterministic endpoint-connected profile extraction, stable boundary selectors, selector-backed exact new-body extrusion, the initial 1,000-point browser budget, complete transient-draft solves, interactive Point/Line/Rectangle/Circle/Arc authoring, P0 constraints and dimensions, profile-region selection, and local sketch undo/redo are executable. Interior-intersection splitting, viewport plane picking, inference glyphs, trim/extend, multi-profile feature input, and add/remove/intersect body semantics remain the next sketch slice. See [ADR-0018](adr/0018-deterministic-sketch-profile-extraction.md), [ADR-0019](adr/0019-selector-backed-new-body-extrusion.md), and [ADR-0021](adr/0021-transient-sketch-draft-solving.md).
