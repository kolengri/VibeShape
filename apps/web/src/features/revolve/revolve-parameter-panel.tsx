...
export function RevolveParameterPanel({
  ...
  return (
    <ParameterPanel
      ...
      message={message}
    >
      <div className="grid min-w-0 gap-1 rounded-md border bg-panel-muted px-3 py-2">
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              type="button"
              className="h-auto min-h-8 w-full min-w-0 justify-start gap-2 overflow-hidden px-2 py-1 text-left"
            ...
            >
              ...
              <span className="min-w-0 truncate">{profileLabels.join(", ")}</span>
            </Button>
          </TooltipTrigger>
        ...
        {profileLabels.map((label, index) =>
          onProfileRemove ? (
            <div key={label} className="flex min-w-0 items-center justify-between gap-2 text-sm">
